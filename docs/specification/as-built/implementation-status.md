...
|------------|------|------|------|-------------|-------|
| RoleType on AuthContext | REQ-06 | `role_types.py`, `auth_models.py` | `test_auth_middleware` (-k role) | — | ADR-014 |
| Bad JWT refused | REQ-05 | `AuthMiddleware` | `test_auth_middleware` | — | 401 UNAUTHORIZED |
| User identity persist | REQ-01 | `user_identity_schema`, repository | `test_user_identity_repository` | — | Alembic `5c8536ec7078` |
| Seed platform_admin + JWT | REQ-01, REQ-47 | `scripts/seed_platform_admin.py`, `AuthIdentityService` | `test_auth_identity_service` | `verify_jwt_login` | Idempotent re-seed |
...
**INIT-GATEFLOW-014 W1 status:** **human_approved** at wave-acceptance — Draft PR [#221](https://github.com/drivestream-lab/gateflow/pull/221) @ `ecb7fbd` label `wave-accepted` (+ Pass-2 tip `f4830f4`); Ground-Report W1 **pass**; Learning-Extract W1 empty items. Detail: [`Implementation-Status-INIT-GATEFLOW-014.md`](Implementation-Status-INIT-GATEFLOW-014.md). Merge/publish at `wave-signoff` only.
...
## Capability matrix (INIT-GATEFLOW-012 W4 — repo-scoped NO_CONCURRENT_RUN)

| Capability | Spec | Code | Unit | Live verify | Notes |
|------------|------|------|------|-------------|-------|
| Org+repo ACTIVE collision | REQ-23 | `RunRepository.find_active_run` (org+repo only) | `test_run_store_concurrency`, `test_implement_concurrent_409` | `verify_wave_start` same-repo 409 | Keeps `PC-06-no-concurrent-active-run` / Conflict 409 |
| Cross-repo never blocks | REQ-24 | same query scoped to requested repo | `test_ff06_cross_repo_*` | optional `GATEFLOW_VERIFY_CROSS_*` | Different repos both eligible |
| No new isolation infra | REQ-25 | query-only broaden | source inspection | N/A | No worktree/lock package added |
| FF-06 before broaden | — | fixture-first | `test_run_store_concurrency` | — | xfail flipped green in TASK-W4-02 |
...
**INIT-GATEFLOW-012 W5 status:** **human_approved** at wave-acceptance — Draft PR [#196](https://github.com/drivestream-lab/gateflow/pull/196) @ `aa6d2bf` label `wave-accepted`; Ground-Report W5 **pass**. Dormant capability (P15 N/A). Merge/publish at `wave-signoff` only. Last eng wave for gateflow half of INIT-012.
