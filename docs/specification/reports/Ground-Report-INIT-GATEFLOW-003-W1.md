...
|----------|----------------------|-------------------|--------|
| REQ-27 (W1) | Live Cursor engineering lane (`pre-implement` → `loop-spec` → `verify` → `ground-spec`) with coding-work evidence; no stub as exit | `verify_engineering_lane` green; RunStore stages/events | **pass** |
...
| D-W0-L1 carry | Live coding work | Laptop spike + Docker bridge + engineering-lane live SDK | **pass** |
...
|----|----|---------|----------|-------------|
| D-W1-L1 | REQ-27 | Live engineering-lane prove-it open at first Draft | — | **Closed 2026-07-25** — `verify_engineering_lane` pass; run `de780ba2-…` |
...
| Engineering-lane live verify | `verify_engineering_lane` | opt-in module | config + worker + key | four stages; `stopped`; evidence | Not in `verify_all` | W2 Scenario A |
