...
| HEAD (at ground) | `0287caa` (+ migration `05abfc0` / Alembic `e7263d04aa6b` on branch) |
...
| REQ-6 | Persist `prompt_id` + `prompt_revision` on packaged stages | ORM/DTO `stages.prompt_*`; orchestrator `StageCreate`; Alembic `e7263d04aa6b`; unit `test_dispatch_persists…` / walker asserts `prompt_id` | **pass** (unit + schema); live assert **deferred** (D-W0-V1) |
...
| Human-owned Alembic revision for new columns | MDC database-migrations | **pass** — `e7263d04aa6b_run_schema_update.py` |
...
| RunStore columns | ORM + DTO + Alembic `e7263d04aa6b` | repository map | nullable text/varchar | `runs.handoff_path`; `stages.prompt_id` / `prompt_revision` | Human-owned migration applied for live | W1 |
...
- [x] Confirm Alembic `e7263d04aa6b` applied on dogfood DB
...
handoff:
  ...
  artifact:
    ...
    digest: sha256:1ed85e921a3b01f7443cbc7240cfff57ff52450157a1a5806bd0ae6204387ffa
  blockers: []
  signals:
    ...
    human_alembic_revision: e7263d04aa6b
