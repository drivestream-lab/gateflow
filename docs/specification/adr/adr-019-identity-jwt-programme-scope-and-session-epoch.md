...
| product_constraints | `[REQ-12, REQ-14, REQ-16]` |
...
| Approval evidence | Explicit PE acceptance by @nikd10x on 2026-08-14 via Cursor chat, Draft spec PR [#243](https://github.com/drivestream-lab/gateflow/pull/243) |
| Approved head | `36d834feb27098ff2942f0aa81676a1b583d8b73` |
| Lint evidence | adr_boundary_lint.py 4/4, PASS, sha256:2fbe1ecdb1a73335982e365dd57bcbff12d4b51e328c95be124cc2efa9e86d25 |
...
Option A). The claim is 1:1; a membership row is N:N. A decoded JWT is not
re-checked against identity row state. ADR-016 attributes `RunSchema.tenant_id`
from the tenant that passed scope. Middleware has no repository; Redis is not
durable session authority (ADR-001). Binds REQ-12, REQ-14, REQ-16.
...
| B — No `tenant_id` on the JWT; scope loads membership `(user_id, path tenant)`; refuse identity-row mismatch on `session_epoch` or inactive status | One mint per credential check; cardinality lives in rows; row change takes effect next request | Extra indexed reads; invert `test_tenant_admin_missing_tenant_id_401` |
| C — Embed all programme ids in the JWT | No membership read on scope | Unbounded claim; membership change still needs remint; no row-state invalidation |
...
`TENANT_ADMIN`. Mint `sub` / `role` / `session_epoch` (plus existing
...
and refuses when row status is inactive or JWT `session_epoch` ≠ row
`session_epoch`. Increment `session_epoch` on credential or status mutation
that must invalidate an already-decoded JWT. `AuthContext.tenant_id` remains
optional and unused for scope. ADR-016 still writes `runs.tenant_id` from the
path tenant after the membership check. Binds REQ-12, REQ-14, REQ-16.
