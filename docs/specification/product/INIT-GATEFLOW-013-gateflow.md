...
| REQ-24 | A connected tenant can refresh its copy of the programme's shared records at any time. Successful refresh also rewrites the Programme `repo_catalogue` snapshot (INIT-014 REQ-49) | PRD REQ-24; CAP-07; US-7; D4 | Refresh call | Synced copy updated; `programmes.repo_catalogue` rewritten from parse | unit + verify | W4 |
...
| REQ-24 | Catalogue refresh fails | Named reason; existing selections and readiness answers untouched; `programmes.repo_catalogue` not rewritten | Refresh must never wipe a working tenant state | unit + verify |
