...
| REQ-08 | `platform_admin` can validate-then-create a **Programme** with meta location and **required** GitHub PAT; workspace root comes from process config ``GATEFLOW_WORKSPACE_ROOT`` (clones at ``{GATEFLOW_WORKSPACE_ROOT}/{org}/{repo}``) | PRD REQ-08; CAP-02; D13, D14, D15, D18 | Onboard call | On success: Programme exists with PAT + workspace + repo catalogue from validation | unit + verify | W1 |
...
| REQ-12 | Workspace root is taken from ``GATEFLOW_WORKSPACE_ROOT`` at programme onboard (not a request-body field) and persisted on the Programme/Tenant for subsequent lifecycle | PRD REQ-12; CAP-02; D15 | Successful onboard | Workspace root persisted and used by subsequent lifecycle; body ``workspace_root`` rejected | unit + verify | W1 |
...
| REQ-48 | After a Programme is created, `platform_admin` can read the meta-derived **repo catalogue** on `GET /api/v1/programmes/{id}` (and list) — same `CatalogueCandidate` shape as create (`org`, `repo`, `service_key`, `status`). Catalogue is persisted at validate-then-create and rewritten on catalogue refresh (REQ-49). Candidates are **not** fleet membership. Select / onboard remain `tenant_admin` (REQ-18, REQ-23) | Issue [#241](https://github.com/drivestream-lab/gateflow/issues/241); CAP-02/CAP-03 | `GET /api/v1/programmes/{id}` | `repo_catalogue` present; PAT absent; `tenant_admin` still forbidden on this route | unit + verify | fix |
| REQ-49 | Catalogue **resync** fetches meta and rewrites `programmes.repo_catalogue`. `platform_admin` uses `POST /api/v1/programmes/{id}/catalogue/refresh` (no Fleet connect required). `tenant_admin` uses existing `POST /api/v1/tenants/{id}/programme/catalogue/refresh` (connect still required) which also persists the snapshot. Git or parse failure is named and fail-closed (no JSONB write, no selection change). PAT never in the response. Select / deselect / onboard stay `tenant_admin` | Issue [#241](https://github.com/drivestream-lab/gateflow/issues/241); INIT-013 REQ-24/25 | Platform or tenant catalogue refresh | Snapshot matches parsed candidates; selections unchanged; PAT absent | unit + verify | fix |
