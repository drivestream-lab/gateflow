...
|----|-------------|-----------|---------------------|---------------|
| FR-15 | Expose authenticated **wave-start API** as the **only** supported start path for 002 programmes; accept wave identity as **ticket/issue id** **or** **initiative_id + wave_id**; if both provided they **must agree** or reject; enqueue work and return `run_id` when wave-run preconditions pass; do not infer intent from board columns; **remove / disable INIT-001 label trigger** for 002 programmes. | PRD §2 FR-15, US-1; Scope boundary; Decision #1/#2; wave-run preconditions | Valid token + agreeing identity + preconditions → 2xx with `run_id` and RunStore run + job; disagreeing dual identity → 400 structured reason, no run; any precondition failure → 409/422 structured reason list, no dispatch; unauthenticated → 401; label-based webhook path does not create runs for 002 programmes; board column change never starts a run. | unit + live verify |
| FR-16 | Resolve **runner + model profile per `workflow_node`** for `dispatch: orchestrated` nodes from gateflow programme config (`model.overrides` and runner override fields); unset nodes use `runner.default` + `model.profiles.default`; persist `runner`, `model_profile`, `model_id`, `model_provider` on every orchestrated stage; do not hardcode node allowlists or change dispatch eligibility (contract `dispatch` remains SSOT). | PRD §2 FR-16, US-3; Programme Config; Decision #5; supersedes INIT-001 FR-13 | ≥ 2 orchestrated nodes can resolve different configured profiles on Cursor path; fields persisted in RunStore; unknown override node / missing profile / unresolvable model → block at start (see FR-18); PolicyEngine unchanged when only override values change. | unit + integration |
...
| FR-18 | **Fail-closed stub / config validation** before enqueue/dispatch: every runner and notifier the run will need must resolve to an **implemented** backend; `notifier.default` (and overrides) must be implemented; invalid per-node runner/model config blocks at start; unused stubs in registry are allowed if not selected; **never** silent fallback to Cursor/GitHub. | PRD §2 FR-18, US-4; Decision #7; Error Handling; wave-run preconditions #10/#11 | Config selecting stub runner or stub notifier required for the run → 422 (or equivalent) with stub id + config key; 0 AgentRunner dispatches; invalid override → structured error with config key / node id. | unit |
...
| 3 | Latest handoff envelope readable per programme config | inherit INIT-001 |
| 4 | `handoff.contract` matches installed contract | inherit |
| 5 | Resolved next node is `type: skill` with `dispatch: orchestrated` | pin `v0.5.0-rc.2` |
| 6 | `handoff.human_checkpoint` not blocking | inherit |
| 7 | No active run for same wave identity / PR/issue scope | FR-15 |
| 8 | Handoff has no unresolved blockers | inherit |
| 9 | `handoff.stage` in wave lane (post-`board-seed`) | pin |
| 10 | Resolved runner + notifier for this run are implemented (not stubs) | FR-18 |
| 11 | Resolved runner/model config for required orchestrated nodes is valid | FR-16 / FR-18 |
...
| A-7 | Programme config (runner/model/notifier/pr naming) stays in **gateflow** repo (ADR-004) | PRD Decision #5; Programme Config | PE | confirmed | Config moved to meta-only |
...
| Q-2 | PE | PR title/body templates and `pr.branch_prefix` defaults (PRD OQ #2 / IM-02) | PE | no | W1 PR-at-start | Add `pr.*` keys to programme config; same naming for success and failure; concrete templates chosen in TDD | open | pending — IM-02 |
