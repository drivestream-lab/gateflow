...
| Q-3 | PE | Concrete `handoff_path` representation (workspace-relative file path vs absolute path vs internal URI / blob key)? Ownership stays Gateflow. PRD §4.4 rule 6 / open item #3 | PE | no | technical review / W0 | `{workspace}/.gateflow/runs/{run_id}/handoff.md` absolute (TDD §3.4) | resolved | TDD §3.4 / ADR-007 |
