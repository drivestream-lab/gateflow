...
| **Gateflow deploy (API + worker)** | **ForgeClient only** (GitHub REST via App installation token preferred) | Callers of `/api/v1/board/*` supply fields; Gateflow does not parse WorkManifest |
...
1. Confirm `APP_ENVIRONMENT=production` rejects `GITHUB_PERSONAL_ACCESS_TOKEN` (ADR-003).
2. Confirm `src/infra_services/forge_client.py` has no `subprocess` / `gh` CLI invocation
   (`make test` → `test_forge_client_source_has_no_gh_subprocess`).
3. Confirm board + PR/comment writes go through `httpx` REST only.
...
Forge-backed create/list/status/link requires a non-prod GitHub token in env.
Without it, `verify_board` still asserts 401 + validation edges.
