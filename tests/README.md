...
| Factory effectiveness (INIT-015 W2) | `python -m tests.verify.verify_factory_effectiveness` (opt-in; not in `verify_all`) | `tests/unit/test_factory_effectiveness_service.py` |
| Delivery scorecard (INIT-015 W3) | `python -m tests.verify.verify_delivery_scorecard` (opt-in; not in `verify_all`) | `tests/unit/test_delivery_scorecard_service.py` |
...
Human live-verify: `.venv/bin/python -m tests.verify.verify_jwt_login` (API up; human-applied Alembic including `58462eaba680`; JWT key material configured; `auth.platform_admin` in `tests/config.yaml`).
