"""FastAPI application setup for gateflow."""
...
async def lifespan(app: FastAPI):
    ...
    yield
    with LoggingContext(startup_correlation_id):
        logger.info("Shutting down gateflow application")
        try:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Gateflow API",
        description="Gateflow orchestrator API — delivery flow runtime for sdd-delivery/v2",
        version="0.1.0",
        lifespan=lifespan,
    ...
    auth_config = AuthConfig(
        ...
        public_paths=["/health", "/internal", "/webhooks", "/api/v1/runs", "/api/v1/metrics"],
